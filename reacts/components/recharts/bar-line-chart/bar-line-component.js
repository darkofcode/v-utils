...
export default function BarChartComponent({ data = [], headers = [], xAxisDataKey = "0" }) {
      ...
      {data.length ? (
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis dataKey={xAxisDataKey} stroke={rechartsColors.green} />
            <YAxis stroke={rechartsColors.green} />
            <Tooltip cursor={false} content={<CustomTooltip />} />
            <Legend formatter={CustomLegend} onClick={handleLegendClick} />
            {headers.slice(1, -1).map((h, i) => (
              <Bar opacity={opacity[i + 1]} key={i} dataKey={i + 1} fill={rechartsColors.colors[i]} />
            ))}
            <Line opacity={opacity[headers.length - 1]} dataKey={headers.length - 1} stroke="#82ca9d" strokeWidth={3} />
          </ComposedChart>
        </ResponsiveContainer>
      ) : (
        <div>No Data</div>
